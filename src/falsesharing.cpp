...
void cacheBounce(unsigned tCount, unsigned N, unsigned nproc) {
     std::vector<std::atomic<int>> counters(tCount);
     for (auto& x: counters) { x.store(0); }
     std::vector<std::thread> threads;
     for (unsigned i = 0; i < tCount; i++) {
          threads.emplace_back(counterBumpThread, &counters[i], N, (int)(i % nproc));
     }
...
int main(int argc, char** argv) {
    ...
    cacheBounce(threads, repetitions, nproc);
    return 0;
}
